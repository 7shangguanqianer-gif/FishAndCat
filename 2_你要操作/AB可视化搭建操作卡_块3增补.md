...
2. 其下建 **Alarm Class**:名 `AC_High`,优先级 0,确认方式 **REP_ACK**
   (⚠️0710 实配修正:AB 2.9 下拉无 "REQ",五个选项 ACK/REP/REP_ACK/ACK_REP/ACK_REP_ACK;
   REP_ACK=恢复+人工确认双条件,最贴"需确认"原意);
3. 建 **Alarm Group**:名 `AG_WH`,组内加一条:Observation type=**Digital**,
   Expression=`GVL_Visu.VisuAlarm`,比较 `= TRUE`,Class 选 `AC_High`,
   Message 填 `Warehouse alarm: no feasible slot / invalid input`;
4. VisuMain 放 **Alarm Banner**(工具箱 报警管理器类):随手拖入 → 属性 Position 填
   X=40 Y=680 W=1200 H=32(⚠️手法:先随手放、再属性面板填精确数,所有元素通用);
   Alarm configuration 组展开 → Alarm groups 取消 All 只勾 `AG_WH`
   (⚠️0710 实配修正:此版**无 "Filter=Newest" 独立属性**,"Filter by latch/time range"
   是别的东西保持空白;单报警场景排序无意义,跳过;列配置用默认即可);
...
> ⚠️0710 修正:AB **IDE 在线预览里 Banner 不渲染是正常的**(官方限制);而 WebVisu 在
> Simulation 下又连不上(§4)——**结论:当前仿真环境里 Banner 无处可眼见**。验收标准降为
> "配置对象齐全 + F11 编译 0 errors";显示效果推迟到真机/软 PLC 阶段验证。
> 演示第 6 步的报警可见性由**红色 Lamp 灯 + ALARM 状态文本**承担(两者在 IDE 在线画面正常显示)。

## 4. WebVisu 发布(⚠️0710 重大修正:Simulation 下无法验证,只做配置不做浏览器验证)

1. 设备树 **Visualization Manager** 右键 → 添加对象 → **WebVisu**;
2. 其属性:Start visualization = `VisuMain`;.htm 名默认 `webvisu.htm`;
3. **F11 编译 0 errors 即算完成,到此为止。不要用浏览器验证!**
   CODESYS 官方论坛证实:IDE Simulation 模式会**禁用设备全部通信(含 WebVisu 的 web 服务器)**
   ——`localhost:8080` 打不开是**架构限制,不是配置错误**,别折腾排障。
4. WebVisu/手机访问的实际验证推迟到三条路之一(待拍板):真机 AC500 / ABB 虚拟 PLC /
   CODESYS Control Win 软 PLC。**录屏主线=IDE 在线画面,不依赖 WebVisu,不受影响。**
> 许可(0710 枪):ABB 官方付费 runtime 功能清单里**无** WebVisu/TargetVisu(推断自带免费,
> 属排除法推断非官方原话);一份老版 Release Notes 有"Remote Target Visu 无许可 30 分钟停"
> 字样、所指未明——最终以真机实测/问 ABB 为准。

## 5. V1-V5 最小验证(0710 修正:Simulation 下 WebVisu 连不上,V1/V2 改 IDE 在线验证,V3 推迟)

| # | 操作(0710 版) | 判定 | 记录到 |
|---|---|---|---|
| V1 | **IDE 在线画面**看 P1(400 格已倍增):登录运行后拖动/刷新 | 流畅=过;卡顿→记录格数阈值(WebVisu 帧率推迟到软PLC/真机档) | 验收截图目录 |
| V2 | 工具箱找 **Path3D** 元素,放空画面绑任意 REAL 数组,**IDE 在线**试渲染 | 能显示=真3D 路线;不能=2.5D 伪3D(已是预案) | 同上 |
| V3 | ~~Banner 在 WebVisu 触发~~ **推迟**:Simulation 下 Banner 无处渲染(IDE 预览不画+WebVisu 连不上,§3/§4) | 本阶段只验"配置齐+编译 0 errors" | — |
| V4 | AB 帮助/许可管理器搜 WebVisu 许可要求(真机档) | 记下原文一句话(与 0710 枪的"付费清单无 WebVisu"交叉验证) | 告诉我 |
...
| 6 | 超重货报警(同存量) | 红灯 + ALARM 状态文本(★Banner 在 Simulation 环境不可见——配置在,真机/软PLC 阶段可见,0710 修正) |
| 7 | 现场调权重(同存量) | δ=0 退化演示 |
| ★8 | ~~浏览器开 WebVisu~~ **推迟**(Simulation 连不上,0710 修正);讲解词改为"画面已配 WebVisu 发布,真机部署即可手机访问" |
...
- [ ] 快照对比:保存→切换,两布局肉眼可辨;
- [ ] Banner 配置对象齐(Alarm Configuration/Class/Group/Banner 元素)+ F11 编译 0 errors
      (0710 修正:Simulation 环境不可眼见,显示验证推迟真机/软PLC);
- [ ] WebVisu 对象已加、编译 0 errors(0710 修正:**不做**浏览器打开验证,Simulation 连不上是正常);
- [ ] V1/V2/V4/V5 四项结果记录并反馈给我(V3 推迟)。
...
| WebVisu 打不开 | **Simulation 下必然打不开(通信被禁用,0710 修正)——不是故障,别排障**;真机/软PLC 下才查:应用必须 Start(Web 服务随应用启停)、端口被占换 8081 |
