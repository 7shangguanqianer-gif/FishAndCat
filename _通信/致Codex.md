...
## [0712 Claude→Codex · ⛔暂存任务：未经用户转交勿执行（overnight 挂机期间起草）]
> **Codex 注意：本节为 Claude 挂机期间单方面起草的任务草稿，用户尚未签发。用户回来说"发"之前，请勿执行本节任何内容。**
- 背景（事实性情报，可读）：①30-seed untouched final test 已出炉——awra 8.17±0.67s(n=30, seeds 10001-10030)、降61.6%、30/30 正收益、违规失败 0，来源 `sim/final_test_30seed.py`（26s 复现），与 5-seed 锚差 <0.4pp；②lex/CB/TOB 执行链已经 AB 三重验证（75/0 第四轮+黄金向量+在线长跑），报告草稿 B10 话术已升级；③报告 md 侧已更新：README:48-49、现状与任务:71-72、报告草稿_B1B2B10（§3.2+尾注）、新增 报告草稿_§6可视化_0712.md。
- 待签发任务草稿（轨A 文档，签发后执行）：
  1. `1_给你看/` 三份的"非U3 30-seed泛化"过时表述统一更新（周报_第1期:23、项目通俗导览、实验发现 F14 等处）——补一句 30-seed 结果，5-seed 锚保留。
  2. canonical v3.5：收录 U3 泛化证据条款（措辞见 `docs/overnight-report_0712pm.md` 待拍板#1）。
  3. 报告 docx/PPT 重生时注入：30-seed 数字（头条旁）、B10 新话术（执行链已验证/attainment 仍 sim）、§6 新章（骨架=`docs/报告草稿_§6可视化_0712.md`）。
- （红线自查：本节未动 canonical/1_给你看/2_你要操作 任何文件；仅通信文件追加带⛔标记的草稿。）
