# -*- coding: utf-8 -*-
"""3D 回放动画 · 1 分钟正式版 v2(0706 深夜;用户三问+自查八项全修复)。

v2 修复账(对照 v1):
1. 路径线=真实轨迹曲线(双轴各自梯形积分的参数曲线,不再是示意直线);
2. 队列箱绑定具体货物(到达即显示自身重量类色;队首拣走全队前移);
3. 入口取货驻留 T_LOAD=4s(与放货 G1 对称;速度曲线两端各踩一个 v=0 平台);
4. KPI「平均响应(本批)」=入库完成-到达时刻(含排队等待,真口径);
5. 故障叙事五拍:第 6 件取货驻留时原目标格(17,2)故障 → 深红+红框+红锥+KPI红
   → 状态行字幕 → 目标线框与轨迹切换到重排格(CSV 真实格=次优解)→ 照常执行
   → 报警"未确认"保持到结束(ISA-18.2);
6. 新增全宽状态字幕条(当前作业/故障插播/收尾语)。
架构不变:事件流驱动,build_events() 可替换为 sim 导出(档 3 完整版)。
...
T_LOAD, T_DWELL = 4.0, 4.0                    # 取货/放货驻留(G1 口径,对称)
FPS, W, H = 15, 1280, 720
PV_BOX = (368, 150, 912, 544)                 # 3D 视口(状态行让出 16px)
DUR = 60.0
FAULT_CELL = (17, 2)                          # 故障戏:原分配目标格
Z0 = -0.5
...
def build_events(goods):
    ...
    每 trip:arrive_t/t0/T_depart/T_xfer/T_place/T_end + 故障戏字段。"""
    batch = goods[11:20]
    trips, t = [], 0.0
    for g in batch:
        T_out = max(axis_T(g["col"], VX, AX), axis_T(g["tier"], VY, AY))
        trips.append(dict(
            g=g, t0=t, T_out=T_out,
            T_pick=t + 0.5 * T_LOAD,             # 队首箱上叉时刻
            T_depart=t + T_LOAD,
            T_xfer=t + T_LOAD + T_out + 0.5 * T_DWELL,
            T_place=t + T_LOAD + T_out + T_DWELL,
            T_end=t + T_LOAD + 2 * T_out + T_DWELL))
        t = trips[-1]["T_end"]
    T_sim = t
    # 到达时刻:前 3 件开场刚到(小负值,不虚增等待);其余泊松间隔
    # (均值 36s:单件服务≈2·T_out+8≈32s → ρ≈0.9 稳定),钳制不晚于拣货
    ...
    for i, tr in enumerate(trips):
        if i < 3:
            ...
            ta += rng.expovariate(1 / 36.0)
            tr["arrive"] = min(ta, tr["t0"] - 1.0)
    # 故障戏:从第 5 趟起选第一件真实格不等于 FAULT_CELL 的货
    ft = next(tr for tr in trips[4:]
              if (tr["g"]["col"], tr["g"]["tier"]) != FAULT_CELL)
    ft["fault"] = True
    fault_t = ft["t0"] + 0.5 * T_LOAD            # 取货驻留中检测到故障
    return trips, fault_t, T_sim
...
def main():
    goods = load_goods()
    trips, fault_t, T_sim = build_events(goods)
    fault_trip = next(tr for tr in trips if tr.get("fault"))
    SPEED = T_sim / (DUR - 1.5)
    FRAMES = int(DUR * FPS)
    print(f"T_sim={T_sim:.1f}s speed={SPEED:.2f}x frames={FRAMES} "
          f"fault_trip={fault_trip['g']['name']} fault_t={fault_t:.1f}")
    ...
    kpi_defs = ["已入库", "到达排队", "累计行程", "平均响应(本批,含排队)",
                "累计能耗", "活动报警"]
    kpi_vals, kpi_cards = [], []
    for i, k in enumerate(kpi_defs):
        ...
        rib.text(x + 0.010, 0.62, k, fontsize=7.5, color="#666",
                 transform=rib.transAxes)
        kpi_vals.append(rib.text(x + 0.010, 0.24, "", fontsize=11, color=C["txt"],
                                 fontweight="bold", transform=rib.transAxes))

    # 状态字幕条(3D 视口上方)
    status = fig.text(0.300, 0.806, "", fontsize=9.5, color=C["txt"], va="center")

    ax_v = fig.add_axes([0.048, 0.560, 0.235, 0.225]); ax_v.set_facecolor(C["panel"])
    ax_q = fig.add_axes([0.048, 0.300, 0.235, 0.200]); ax_q.set_facecolor(C["panel"])
    ax_b = fig.add_axes([0.048, 0.048, 0.235, 0.192]); ax_b.set_facecolor(C["panel"])

    # 队列曲线:队列=已到达未上叉的货(集合语义,与 3D 队列箱同源)
    def queue_members(t):
        return [tr for tr in trips if tr["arrive"] <= t < tr["T_pick"]]

    ev_t = sorted({0.0} | {tr["arrive"] for tr in trips}
                  | {tr["T_pick"] for tr in trips} | {T_sim})
    ev_t = [x for x in ev_t if x >= 0]
    q_steps_t = ev_t
    q_steps_n = [len(queue_members(x + 1e-6)) for x in ev_t]
    ...
    ax_q.set_title("入口到达队列长度(泊松到达,ρ≈0.9)", fontsize=8.5,
                   color=C["txt"], pad=3)
    ...
    dws = ax_b.bar(range(20), [T_LOAD + T_DWELL if i < 11 else 0 for i in range(20)],
                   bottom=trav0, color="#C9CDD2", width=0.62)
    ax_b.set_xticks(range(20)); ax_b.set_xticklabels(names, fontsize=5.5)
    ax_b.set_ylim(0, 26)
    ax_b.set_title("单件服务时长分解 s(行程+两端装卸;琥珀=在途)", fontsize=8.5,
                   color=C["txt"], pad=3)
    ...
    def redraw_speed(tr, show_cell=None):
        ...
        ts = np.linspace(0, T_tot, 420)
        def prof(t, d, vm, ac):
            if t < T_LOAD:
                return 0.0
            if t <= T_LOAD + T_out:
                return axis_v(t - T_LOAD, d, vm, ac)
            if t <= T_LOAD + T_out + T_DWELL:
                return 0.0
            return axis_v(t - T_LOAD - T_out - T_DWELL, d, vm, ac)
        vx = [prof(t, dx, VX, AX) for t in ts]
        vz = [prof(t, dz, VY, AY) for t in ts]
        ax_v.plot(ts, vx, color=C["navy"], lw=1.6, label="水平 vmax=2.0(A4)")
        ax_v.plot(ts, vz, color=C["mid"], lw=1.6, ls="--", label="垂直 vmax=0.5(A4)")
        ax_v.plot([0, T_LOAD], [0, 0], color=C["amber"], lw=3.0,
        ...
        ax_v.legend(fontsize=6.2, framealpha=0.9, loc="upper right")
        for s in ax_v.spines.values():
            s.set_color("#CCCCCC")
        speed_state["cursor"] = ax_v.axvline(0, color=C["amber"], lw=1.4)
        speed_state["trip"] = tr
    ...
    fpath = pl.add_mesh(pv.lines_from_points(traj_points(*FAULT_CELL)),
                        color=C["path"], line_width=3)
    alarm_box = pl.add_mesh(box(FAULT_CELL[0] + 0.14, FAULT_CELL[0] + 0.86,
                                0.12, 0.88, FAULT_CELL[1] + 0.14,
                                FAULT_CELL[1] + 0.86), color=C["alarm"])
    alarm_wire = pl.add_mesh(box(FAULT_CELL[0] + 0.03, FAULT_CELL[0] + 0.97,
                                 -0.03, 0.05, FAULT_CELL[1] + 0.03,
                                 FAULT_CELL[1] + 0.97),
                             color=C["alarm"], style="wireframe", line_width=5)
    alarm_cone = pl.add_mesh(pv.Cone(center=(FAULT_CELL[0] + 0.5, 0.5,
                                             FAULT_CELL[1] + 1.7),
                                     direction=(0, 0, -1), height=0.7,
                                     radius=0.30), color=C["alarm"])
    ...
    for i in range(FRAMES):
        ...
        qmem = queue_members(t)
        fault = t >= fault_t
        t_travel = base_travel + sum(x["T_out"] for x in trips if t >= x["T_xfer"])
        done_batch = [x for x in trips if t >= x["T_xfer"]]
        resp = (sum(x["T_xfer"] - x["arrive"] for x in done_batch)
                / len(done_batch)) if done_batch else None
        ...
        q_line.set_data(q_steps_t[:k] + [t], q_steps_n[:k] + [len(qmem)])
        cur_q.set_xdata([t, t])
        for j, x in enumerate(trips):
            ...
            if t >= x["T_xfer"]:
                ...
                dws[gi].set_y(x["T_out"]); dws[gi].set_height(T_LOAD + T_DWELL)
            elif tr is x:
                bars[gi].set_height(x["T_out"])
                bars[gi].set_color(C["amber"])
        vals = [f"{n_placed}/20 件", f"{len(qmem)} 件", f"{t_travel:.1f} s",
                f"{resp:.1f} s" if resp else "—",
        ...
        # 状态行
        if t >= T_sim:
            status.set_text("演示批 20 件完成 · 队列剩余货物持续到达(档 3:系统不停机)")
            status.set_color(C["txt"])
        elif tr is fault_trip and fault_t <= t < fault_t + 14:
            status.set_text(f"▲ 库位{FAULT_CELL} 故障 → 在线重排至 "
                            f"({tr['g']['col']},{tr['g']['tier']})(次优解,作业不中断)")
            status.set_color(C["alarm"])
        elif tr is not None:
            g = tr["g"]
        ...
        for j, x in enumerate(trips):
            g = x["g"]
            in_q = x["arrive"] <= t < x["T_pick"]
            qboxes[j].SetVisibility(in_q)
            if in_q:
                idx = qmem.index(x)
                qboxes[j].SetPosition(0.5, -2.0 - min(idx, 4) * 1.1, Z0 + 0.42)
            if tr is x:
                tt = t - x["t0"]
                if tt < T_LOAD:
                    cx, cz, ph = 0.0, 0.0, "load"
                elif tt <= T_LOAD + x["T_out"]:
                    cx = axis_pos(tt - T_LOAD, g["col"], VX, AX)
                    cz = axis_pos(tt - T_LOAD, g["tier"], VY, AY)
                    ph = "out"
                elif t <= x["T_place"]:
                ...
                carried[j].SetVisibility(x["T_pick"] <= t < x["T_xfer"])
                is_fault_wait = (x is fault_trip and not fault)
                tgt.SetPosition(FAULT_CELL[0] if is_fault_wait else g["col"], 0,
                                FAULT_CELL[1] if is_fault_wait else g["tier"])
        ...
        if i % 100 == 0:
            print(f"frame {i}/{FRAMES} t={t:.1f}s placed={n_placed} q={len(qmem)}")
    gen.close()
