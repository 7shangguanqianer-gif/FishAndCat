...
slide("拟真度阶梯:三档同管线验证(L7)",
      [("档1 数据模型(匀速+固定权重)→ 档2 工程主口径 H → 档3 工业场景", 0),
       ("档3 = 泊松到达峰谷 + 货叉装卸 + 频次估计噪声 + 故障注入(参数显式,canonical G)", 1),
       (f"降幅阶梯:{br.TIER_DROP['1']}% → {br.TIER_DROP['2']}% → {br.TIER_DROP['3']}%"
        "——匀速口径高估约 7pp,工业场景优势稳定;排序不变、违规 0", 0),
       (f"档3 排队视角:响应 P95 {br.RESP_SEQ95}s → {br.RESP_AW95}s"
        "——布局优化直接改善高负载可用性", 1),
       ("频次画像带噪(σ=0.3)AWRA 布局质量不塌:多因子 Rank+局部搜索兜底", 1)],
      fig="fig16_拟真度阶梯.png",
      note="回应'贴合实际吗':不是堆模型层数,而是证明每加一层现实结论都站得住。")
