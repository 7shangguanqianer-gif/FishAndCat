...
def cycle_energy_joule(s_in, s_out):
    ...
    联程腿:空载台架 在存位→取位间移动(高度差取上升部分,下降不回收=保守);
    取腿:载货台+货物 从取位回 I/O(下降不回收,水平按摩擦计)。"""
...
def main():
    ...
    print("  呼应:ABB 运动控制事业部'节能增效、脱碳'背景与'绿色工程视角'评委讲话;"
          "下降再生制动(ABB 传动方案)可进一步回收,作展望不计入数字(保守)。")
