...
年化(混合作业流口径,L2):按 DAILY_OPS 个单操作/日 × WORK_DAYS 日/年缩放；
500 个单操作=250 个双命令周期，避免 operation/cycle 歧义。

对比:seq 题面基线布局 vs AWRA-LS 布局(同一混合负载,B6),差值=优化带来的年节省。
运行:python energy_report.py
输出:out/energy_report_data.csv + out/energy_report_params.csv
     + figs/fig10_绿色换算.png + 控制台假设块与结论
"""
import csv
import os
import sys
...
DAILY_OPS = 500     # 作业强度假设:每日单操作数(存+取合计=250 双周期/日)
WORK_DAYS = 300     # 年工作日
ELEC_PRICE = 0.80   # 工业电价 元/kWh(假设)
CO2_FACTOR = 0.5306 # 2023 年全国电力平均二氧化碳排放因子 kgCO2/kWh；
                    # 生态环境部公告 2025 年第47号。不含市场化非化石口径为0.6096，
                    # 本项目无该特殊购电边界，故采用全国平均值。
...
def main():
    ...
    print(f"  作业强度 {DAILY_OPS} 个单操作/日(=250 双周期/日) × {WORK_DAYS} 日/年")
    print(f"  电价 {ELEC_PRICE} 元/kWh | 2023 全国平均排放因子 {CO2_FACTOR} kgCO2/kWh")
    print("=" * 62)
    ...
    param_path = os.path.join(HERE, "out", "energy_report_params.csv")
    with open(param_path, "w", newline="", encoding="utf-8-sig") as fp:
        ...
        for k, v, note in [
                ("eta", ETA, "电机+传动综合效率假设"),
                ("mu", MU, "水平等效阻力系数假设"),
                ("carrier_kg", M_CARRIER, "载货台+货叉自重假设"),
                ("daily_single_ops", DAILY_OPS, "单操作/日；等于250双周期/日"),
                ("work_days", WORK_DAYS, "年工作日假设"),
                ("elec_price_yuan_kwh", ELEC_PRICE, "工业电价情景假设"),
                ("co2_kg_per_kwh", CO2_FACTOR,
                 "2023全国平均；生态环境部公告2025年第47号"),
                ("energy_scope", "sim_net_mechanical_work",
                 "sim净机械功情景换算；未计待机/辅助/动态损耗"),
                ("eta_regen_outlook", ETA_REGEN, "展望口径；不进头条")]:
            w.writerow([k, v, note])
    path = os.path.join(HERE, "out", "energy_report_data.csv")
    ...
    print(f"输出:{path} + {param_path} + figs/fig10_绿色换算.png")
