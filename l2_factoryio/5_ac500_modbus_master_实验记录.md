...
- F5 状态：**已结案(Codex 二审 0714 12:22, `_通信/codex_out/0713_F5二审.md`)**。裁决=部分接受证据解释+接受项目级停止条件:
  **当前安装环境 VirtualAC500 主站直驱不可用**(五要点重做后合法 FC2 仍同步返回 `ERR_INTERNAL_UNEXPECTED_STATE` 16#8005,502/5020 无连接层差分,失败早于远端 TCP 阶段)。
  **保守口径**:不得称"全 VirtualAC500 版本永不可用",对外写"当前实例未能提供该主站功能,项目按不可用处理"。
  **C4/C5/F6**:停止在 VirtualAC500 内试参数;转 PM5650 实机或 CODESYS/软 PLC 独立联调替身(严格标注平台)。详见 codex_out 二审全文的"未试配置面逐项裁决"。
