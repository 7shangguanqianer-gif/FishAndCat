...
| 02_GVL.st | 3 个 GVL | GVL_Param(常量段+变量段合一个)/ GVL_WH / GVL_Visu |
| 03_Functions.st | 9 个 POU-函数 | FC_CapCoef 等纯函数,无状态可单测 |
| 04_FB_Warehouse.st | 6 个 POU-功能块 | Init / SelectSlot / AssignAllGoods / LocalSwapImprove / Stats / BuildVisuPath |
| 05_PRG_Main.st | 1 个 POU-程序 | 主状态机,挂循环任务 |
| 06_PRG_Test.st | 1 个 POU-程序 | 21 个边界+一致性用例 |
| 07_GVL_Data_generated.st | 1 DUT + 1 GVL + 1 函数 | **生成文件勿手改**,由 sim/export_st_vectors.py 重生 |
...
5. 首次验证顺序:
   a. PRG_Test.xRunTests := TRUE → 期望 iPassed=21, iFailed=0(T19/T20 是与 Python 的一致性向量);
