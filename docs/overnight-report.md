...
派 fresh-context 红队(opus/high)独立从 30 seed 原始 JSON **逐个复算** M2/M4a 全部数字与机理。结论:
- **所有 headline 数字、配对数、五判据裁决、负结果结论——全部复现一致、站得住**,可作报告依据。
- 揪出并**已订正** 3 处机理/标注错(不动生产代码与已落盘证据数值):
  1. whatif §1 dynfmax 动机「max freq≈28 致 f_norm 塌缩」= 事实错(实测 max freq=100 占 23/30 seed、余 41–97)→ 已改为「猜测…经实测证否」。
  2. whatif §5 dynfmax 机理「排序不变旋钮」= 证伪(6/30 seed 放置确实变;均值≈0 真因是 23/30 seed max freq 撞常数 100)→ 已改真机理,结论不变。
  3. 双命令 §2「单命令总时」列原为反推值 → 已换真均值(seq 7411);§3「基线贵→省%高」因果降级为几何比值解释 + 点出 near 非单调反例。
- 订正已同步到 `报告周素材_0718增补包.md` §2。

## 七、增量:M4b 02页起步(本夜额外完成)

用户「继续」后,验证前置(45 trace/manifest/three 依赖齐)后**实建** `02_入出闭环.html`:基于原宿主页(冻结页复制,非覆盖),**浏览器实测运行时可启**(0 错误/120 库存/trace 驱动/五 lane),应用核心治理(入出闭环命名 + AUTO 精确话术「规则路由 启动时一次选定」+ 禁词零命中),配 boot-QA 8/8。**A 壳 rail 布局/cycle 轴(0-25-50-75-100)/取货实测块留用户在场窗口迭代**(承绿框教训:新页视觉宜共迭代;详 implementation-notes)。

**建议**:验收 M1 后可走 /wrapup 五联动;M4b 页体在场窗口继续(4 待澄清)、M6 分镜定后我渲染。
