...
- **0714 午后 · 算法层 5 缺口全部补跑完善(F1-F5)+ Codex 队列 4 回执确认**:
  - **F1(真相源诚实硬伤)**:新增 `sim/lifecycle_stats.py`(scipy 配对 t 检验)——U5 主组三档 p 全 <1e-5(***,即 p<0.001 成立)、240 高压组全 ns,降幅 +2.69/4.31/4.53% 逐位吻合 canonical +2.7/4.3/4.5%。build_report §2.3 的裸串 "p<0.001" 改为 sourced `LC_P`(从 lifecycle_stats.csv,并注明"240 组多为 ns")。**"p<0.001" 从无据裸串变为可复现计算**。
  - **F2(口径披露缺口)**:instance_gen.py/oracle_gap.py 加 `--accel`(默认关不动 legacy,开则 ws.ACCEL=True + `_accel` 输出)。**H 口径(梯形加减速)lex attainment=99.19% vs 匀速 98.5%——跨口径稳健(反而更高)**。build_report §6.4 加口径披露("匀速口径 98.5%/H 口径 99.19%,各属其口径不可混引")。legacy 文件零污染,pytest 32 passed。
  - **F5(scope 披露)**:build_report §1.1 头条加"120 件·skew 偏态分布[电工物料典型画像]·sum,为头条固定口径;uniform/heavy 另见 §4"。
  - **F3(引用无脚本)**:新增 `sim/cb_st_equiv.py`——独立复刻 ST(04_FB_Warehouse.st)CB 放置分支,对照 Python 参考实现,rule{sum,and}×n{1,3,20,60,120,200,260}×30seeds=**420/420 一致 mismatch=0**,补上"420 零分歧"此前无 committed 脚本的缺口(诚实标注=ST 分支的 Python 独立复刻,非 AB 实跑)。
  - **F4(数据未接主报告)**:scan_g2g4/correlated_storage 数据已存在有效、脚本 py_compile 通过(可复现);**全报告 §-集成卡在 B6 章节归属(§4.7 vs 4.3)的用户拍板**——非代码问题,待用户定章节位置。
  - **验证总账**:build_report 147 数字(原 145+LC_P+LEX_ATTAIN_ACCEL)待补 0 exit0;pytest 32 passed;修了一个因输入路径被误接 OUT 而破坏 test_oracle_gap 的回归(输入回退 DETAIL 冻结路径)。commit 7495c64(F1F2F5)+51e2306(F3)。
  - **canonical 同步待用户批**(红线不碰 canonical):U5 证据指针建议加 `lifecycle_stats.py`(让 p<0.001 可溯源)——见给用户的 canonical 提案。
  - **Codex 队列 4 回执确认**(用户告知已全完成):①F5 二审(已落盘,当前环境负结果);②面板重构(桌面面板已换暗色 SCADA 版,含正确 F3 降级/F5 结案口径,15 时间戳/21 本地链接/零外网);③HTML 文案池(`codex_out/0714_HTML文案池.md`:六章故事线+评分映射+21 HMI 热点);④PPT 重生预研(`codex_out/0714_PPT重生预研.md`:18→20 页方案+旧口径清单+验收门,未改 PPT/WORD 时间戳)。**下一硬门=用户验收 WORD 后才开 PPT 重生**。
  - 反思行:①"能一键复现"≠"统计断言有依据"——F1 的 p<0.001 数字能复现但断言无计算,溯源自检只查"数字有源"不查"断言有据",已补;②F2 证明关键数字应带**口径标签**(匀速 vs H),不同口径的同名指标必须分别标注、禁混引;③F4 提示"完善"有用户拍板边界——代码能做的做完,章节归属这类决策留给用户不越权。
