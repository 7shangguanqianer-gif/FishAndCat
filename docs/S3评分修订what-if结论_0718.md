...
- **dynfmax 近乎无效,但机理不是「排序不变」**(此点经两轮对抗复算订正):①**24/30 seed** 与 baseline **逐项完全相同**(其中 23 个批次 max freq 恰为 100=生产常数;另 1 个 seed 2051 的 f_dyn=97 但放置恰未变);②另 **6/30 seed**(seeds 2027/2035/2039/2041/2046/2050,f_dyn 41–82)放置**确实改变**——f_norm 同时进 efficiency(系数 +f_norm)与 position_reservation(系数 1−f_norm),两项符号相反,缩放 f_max 会**重配二者权重**而非全局线性缩放,可翻转局部择格。24+6=30。两类叠加使四指标均值≈0(expected −0.00%、hot20 −0.04%),但同表 heavy20 **+0.23%**、lift **+0.19%** 已非零,正是「放置确实变了」的证据。**结论不变(dynfmax 帮不上主指标),但它靠的是「多数 seed 的 max freq 撞上常数 100」,而非评分排序不变。**
