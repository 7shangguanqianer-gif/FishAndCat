...
观感/设计质疑两点催生修订假设:①冷门首件被送到最远角(观感过激);②**猜测**常数 f_max=100 在 skew 下偏大、压低 f_norm 使效率项失活(**此前提后被实测证否**:30 seed 中批次实际 max freq 恰为 100 的占 **23/30**、其余 41–97,f_norm 并未塌缩——见 §五②)。对应:

| 变体 | 改动 | 预期收益 |
|---|---|---|
| **V1 τ** | 位置保留距离饱和:g=min(t_norm,τ)/τ,τ=0.5 | 冷门货「推到中远即止」,不再区分中远与最远角 → 首件不再送最远角 |
| **V2 dynfmax** | f_norm 分母=批次实际 max freq(替代常数 100) | (拟)修「f_norm 塌缩致效率项失活」——但该前提实测不成立(§五②) |
| **V3 both** | 两者叠加 | — |
...
- **dynfmax 近乎无效,但机理不是「排序不变」**(此点经对抗复算订正):①**23/30 seed** 批次实际 max freq 恰为 100(=生产常数),这些 seed 下 dynfmax 与 baseline **逐项完全相同**;②其余 **6/30 seed**(f_dyn 41–97)放置**确实改变**——f_norm 同时进 efficiency(系数 +f_norm)与 position_reservation(系数 1−f_norm),两项符号相反,缩放 f_max 会**重配二者权重**而非全局线性缩放,可翻转局部择格。两类叠加使四指标均值≈0(expected −0.00%、hot20 −0.04%),但同表 heavy20 **+0.23%**、lift **+0.19%** 已非零,正是「放置确实变了」的证据。**结论不变(dynfmax 帮不上主指标),但它靠的是「多数 seed 的 max freq 撞上常数 100」,而非评分排序不变。**
