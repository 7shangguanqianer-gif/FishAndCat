...
## 四、待拍板点（集中）
1. sim/out 关键 CSV 附不附（推荐附，省评委时间 vs 包体积）。
2. README 评委版改写的时机与执行者（建议报告周由 Claude 起草另存新文件，不动现 README）。
3. tools/ 是否以附录形式展示（体现工程化，风险=暴露内部流程复杂度）。
4. 双备份形态：单 zip vs "工程包+复现包"两个 zip（推荐后者，评委各取所需）。
