...
> **历史快照说明**：本回执记录HEAD `86800fb`时的审查事实。其后Claude并发修改了near平局、CB/TOB/lex路由和T60-T69测试；正文旧file:line与“未实现”结论不得当作当前状态，最新证据边界仍为59/0，见`docs/canonical_assumptions.md:C11`。
...
| **N5-25** 报告草稿 546.0→278.9 却写省 266.5 内部矛盾 | **接受(当时未修复；现已修复)** | 中 | `docs/报告草稿_C3C5B14_0712.md` 已改为 545.4/278.9/266.5；权威数据现位于 `sim/out/energy_report_data.csv`（旧双段文件已迁移） | 本人亲验：546.0−278.9=267.1≠266.5；545.4−278.9=266.5✓。本行保留 N5 发现的历史证据，现状以拆分后的 data/params 文件及当前草稿为准 | 统一"545.4→278.9,省266.5"或全取整"545→279,省266"，禁混精度相减 |
...
| **N5-33** 6 份 CSV 一文件内塞两种 schema | **接受；现已修复** | 中 | 生成侧已拆为 `*_data.csv` + `*_params.csv`（`reslot_cycle`/`rl_probe` 使用 `*_summary.csv`）；本行所述 `rl_probe.csv` 为修复前历史文件 | 亲读 5 处生成码；原探针精确匹配“同宽度、零结构信号、语义静默改变”。旧双段文件现已退役，拆分 schema 由 `test_method_integrity.py` 锁定 | 保留本行作为 N5 历史证据；当前消费端只读拆分后的标准 CSV |
