...
RESULT = r"F:\abb_wh_spike\visu_native_20260710_1225\build_visu_pages_formal_result.txt"
EXPORT_MAIN = r"F:\abb_wh_spike\visu_native_20260710_1225\VisuMain_formal_after.xml"
EXPORT_STATS = r"F:\abb_wh_spike\visu_native_20260710_1225\VisuStats_formal_after.xml"
...
add_box(visu_main, 40, 4, 560, 30, u"智储优控 · 立体仓储仓位优化")
...
add_dynamic(visu_main, 620, 40, 560, 30, u"状态: %s", "GVL_Visu.VisuStatusText")
add_lamp(visu_main, 1200, 38, 32, 32, "GVL_Visu.VisuAlarm")

inputs = [
    (90, u"ID", u"%d", "GVL_Visu.InGoodId", "Numpad", u"1", u"", u"货物 ID"),
    (122, u"名称", u"%s", "GVL_Visu.InGoodName", "Keypad", u"", u"", u"货物名称"),
    (154, u"重量", u"%.1f", "GVL_Visu.InGoodWeight", "Numpad", u"0", u"200", u"重量 kg"),
    (186, u"频次", u"%.1f", "GVL_Visu.InGoodFreq", "Numpad", u"0", u"100", u"访问频次"),
    (218, u"体积", u"%.2f", "GVL_Visu.InGoodVolume", "Numpad", u"0", u"1.0", u"体积 m³"),
]
for row in inputs:
    add_labeled_input(visu_main, 620, *row)

for x, width, text, variable in [
        (620, 100, u"添加货物", "GVL_Visu.CmdAddGood"),
        (730, 100, u"载入演示", "GVL_Visu.CmdLoadDemo"),
        (840, 80, u"复位", "GVL_Visu.CmdReset")]:
    button = add_button(visu_main, x, 270, width, 34, text)
    add_tap(visu_main, button, variable)

add_box(visu_main, 620, 320, 50, 34, u"策略")
for x, width, text, value in [
        (680, 70, u"顺序", 0), (760, 70, u"就近", 1),
        (840, 70, u"评分", 2), (920, 100, u"AWRA-LS", 3)]:
...
add_dynamic(visu_main, 1030, 320, 230, 34,
            u"当前策略: %d", "GVL_Visu.SelStrategy")

for x, text, variable in [
        (620, u"运行分配", "GVL_Visu.CmdRunAssign"),
        (730, u"动画回放", "GVL_Visu.CmdAnimate"),
        (840, u"保存快照", "GVL_Visu.CmdSaveSnap")]:
    button = add_button(visu_main, x, 380, 100, 34, text)
    add_tap(visu_main, button, variable)
compare = add_button(visu_main, 950, 380, 100, 34, u"实时/快照")
...
add_box(visu_main, 1098, 384, 100, 24, u"快照有效")
to_stats = add_button(visu_main, 1140, 270, 120, 34, u"P2 统计查询")
add_navigation(visu_main, to_stats, "VisuStats")

add_labeled_value(visu_main, 620, 430, u"目标 X", u"%d", "GVL_Visu.VisuTargetX", 70, 50)
add_labeled_value(visu_main, 750, 430, u"目标 Y", u"%d", "GVL_Visu.VisuTargetY", 70, 50)
add_labeled_value(visu_main, 880, 430, u"路径 m", u"%.1f", "GVL_Visu.VisuPathLen", 74, 64)
add_labeled_value(visu_main, 1030, 430, u"行程 s", u"%.1f", "GVL_Visu.VisuExecTime", 74, 64)

for y, label, text, variable in [
        (480, u"总入库时间 / s", u"%.1f", "GVL_Visu.VisuTotalTime"),
        (508, u"平均单件 / s", u"%.1f", "GVL_Visu.VisuAvgTime"),
        (536, u"空间利用率 / %", u"%.1f", "GVL_Visu.VisuSpaceUtilization"),
        (564, u"路径总长 / m", u"%.1f", "GVL_Visu.VisuWearIndex"),
        (592, u"约束违规 / 次", u"%d", "GVL_Visu.VisuViolationCount")]:
...
for x, label, variable, default in [
        (620, u"α效率", "GVL_Param.rAlpha", u"1.0"),
        (750, u"β稳定", "GVL_Param.rBeta", u"0.6"),
        (880, u"γ能耗", "GVL_Param.rGamma", u"0.4"),
        (1010, u"δ预留", "GVL_Param.rDelta", u"0.15")]:
    add_box(visu_main, x, 626, 62, 26, label)
    add_input(visu_main, x + 68, 626, 58, 26, u"%.2f", variable,
              "Numpad", u"0", u"", label)

add_box(visu_main, 620, 654, 270, 18, u"默认: α=1.0 β=0.6 γ=0.4 δ=0.15")
add_box(visu_main, 900, 654, 360, 18,
        u"在线测试 45/45 · sim-ST 双实现互证 · 驻留点=I/O(400格枚举最优)")
...
add_box(visu_main, 40, 466, 560, 20,
        u"列 0~19 →   层0在底层(承重最大)，I/O 口在左下角")
legend = [u"0 空闲(浅灰)", u"1 预占(深灰)", u"2 占用(绿)",
          u"3 推荐(黄)", u"4 路径(蓝)", u"5 报警(红)", u"6 查询(青)"]
...
add_box(visu_stats, 40, 16, 400, 30, u"P2 · 记录、查询与拟真参数")

headers = [(40, 90, u"货物 ID"), (150, 90, u"列 X"), (260, 90, u"层 Y"),
           (370, 110, u"路径 / m"), (500, 110, u"行程 / s")]
...
add_box(visu_stats, 40, 378, 570, 24,
        u"表格按 aGoods 记录翻页；ID 是业务编号，不等于数组行号。")
up = add_button(visu_stats, 40, 410, 100, 34, u"上一页")
add_tap(visu_stats, up, "GVL_Visu.CmdPageUp")
down = add_button(visu_stats, 150, 410, 100, 34, u"下一页")
add_tap(visu_stats, down, "GVL_Visu.CmdPageDown")
add_dynamic(visu_stats, 270, 410, 160, 34, u"首行: %d", "GVL_Visu.iRecTop")
add_dynamic(visu_stats, 440, 410, 170, 34, u"总件数: %d", "GVL_Visu.iRecTotal")

add_box(visu_stats, 650, 16, 570, 30, u"按业务 ID 查询（支持 901/902/903 等演示编号）")
add_box(visu_stats, 650, 60, 130, 30, u"查询 ID")
add_input(visu_stats, 790, 60, 170, 30, u"%d", "GVL_Visu.InQueryId",
          "Numpad", u"1", u"", u"查询货物 ID")
query = add_button(visu_stats, 970, 60, 100, 30, u"查询")
add_tap(visu_stats, query, "GVL_Visu.CmdQuery")
add_lamp(visu_stats, 1080, 60, 32, 30, "GVL_Visu.QryFound")
add_box(visu_stats, 1118, 60, 102, 30, u"查询命中")

query_rows = [
    (104, u"名称", u"%s", "GVL_Visu.QryName"),
    (132, u"重量 / kg", u"%.1f", "GVL_Visu.QryWeight"),
    (160, u"频次", u"%.1f", "GVL_Visu.QryFreq"),
    (188, u"体积 / m³", u"%.2f", "GVL_Visu.QryVolume"),
    (216, u"位置 X", u"%d", "GVL_Visu.QryX"),
    (244, u"位置 Y", u"%d", "GVL_Visu.QryY"),
    (272, u"路径 / m", u"%.1f", "GVL_Visu.QryPathLen"),
    (300, u"行程 / s", u"%.2f", "GVL_Visu.QryExecTime"),
    (328, u"载货行程 / s", u"%.2f", "GVL_Visu.QryTravelLaden"),
    (356, u"装卸时间 / s", u"%.2f", "GVL_Visu.QryHandleT"),
]
for y, label, fmt, variable in query_rows:
    add_labeled_value(visu_stats, 650, y, label, fmt, variable, 150, 250)

add_box(visu_stats, 650, 400, 570, 26, u"拟真参数（修改后请再次按“查询”刷新结果）")
tuning = [
    (434, 650, u"垂直载荷折减", "GVL_Param.rLadenFactorVY", u"0", u"1"),
    (466, 650, u"轻/中重量界 W1", "GVL_Param.rHandleW1", u"0", u"200"),
    (498, 650, u"中/重重量界 W2", "GVL_Param.rHandleW2", u"0", u"200"),
    (434, 950, u"轻载装卸 T1", "GVL_Param.rHandleT1", u"0", u""),
    (466, 950, u"中载装卸 T2", "GVL_Param.rHandleT2", u"0", u""),
    (498, 950, u"重载装卸 T3", "GVL_Param.rHandleT3", u"0", u""),
]
...
add_box(visu_stats, 40, 472, 570, 28, u"当前批次汇总")
for y, label, fmt, variable in [
        (506, u"总入库时间 / s", u"%.1f", "GVL_Visu.VisuTotalTime"),
        (534, u"平均单件 / s", u"%.1f", "GVL_Visu.VisuAvgTime"),
        (562, u"空间利用率 / %", u"%.1f", "GVL_Visu.VisuSpaceUtilization"),
        (590, u"路径总长 / m", u"%.1f", "GVL_Visu.VisuWearIndex"),
        (618, u"约束违规 / 次", u"%d", "GVL_Visu.VisuViolationCount")]:
    add_labeled_value(visu_stats, 40, y, label, fmt, variable, 190, 180)

back = add_button(visu_stats, 1100, 640, 120, 34, u"返回 P1")
add_navigation(visu_stats, back, "VisuMain")
add_box(visu_stats, 40, 688, 1180, 28,
        u"Alarm Banner 区域：Simulation 下不可见属已知限制；批次 D 以配置齐全 + 编译 0 错验收。")
