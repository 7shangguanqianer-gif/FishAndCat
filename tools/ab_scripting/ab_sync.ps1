...
if (-not $SyncOnly) {
    ...
    if ($out2 -match "iPassed = INT#59" -and $out2 -match "iFailed = INT#0") {
        Write-Output "=== ALL GREEN: iPassed=59 iFailed=0 ==="
    } else {
        Write-Output "!! online test not 59/0 (full log: runtest_result.txt)"
        exit 2
    }
}
