...
if (-not $SyncOnly) {
    ...
    if ($out2 -match "iPassed = INT#77" -and $out2 -match "iFailed = INT#0") {
        Write-Output "=== ALL GREEN: iPassed=77 iFailed=0 ==="
    } else {
        Write-Output "!! online test not 77/0 (full log: runtest_result.txt)"
        exit 2
    }
}
